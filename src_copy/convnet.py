...
A Theano-based program for training and running basic convolutional
neural networks.

Supports several layer types (fully connected, convolutional, max
pooling, softmax), and activation functions.
...
class Network(object):
    ...
    def feedforward(self, data, conv_shape):
        data = theano.shared(
        np.asarray(data, dtype=theano.config.floatX), borrow=True)

        cur_layer = self.layers[0]
        data = data.reshape((1,)+cur_layer.image_shape[1:])
        output = conv.conv2d(data, cur_layer.w,
            image_shape = (1,)+cur_layer.image_shape[1:],
        ...
        data = output.eval()
        output = sigmoid(np.dot(data.reshape(conv_shape,), cur_layer.w.get_value())+cur_layer.b.get_value())
